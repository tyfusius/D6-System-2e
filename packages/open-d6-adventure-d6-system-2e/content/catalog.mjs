const book = "D6 Adventure";
const source = (page) => ({ book, page });
const slug = (value) =>
  value
    .toLowerCase()
    .normalize("NFKD")
...
const skills = Object.entries(groups).flatMap(([attributeId, names]) =>
  names.map((name) => ({
    attributeId,
    key: slug(name),
    name,
...
const rankFrom = (name) => Number(name.match(/\(R?(\d+)/u)?.[1] ?? 1);
const trait = (name, type, page, cost = rankFrom(name)) => ({
  name,
  sourcePage: page,
  system: {
    activation: "",
...
const disadvantages = [
  ...
  trait(name, "disadvantage", name.startsWith("Age") ? 15 : 16, 0),
);
...
const gear = (name, availability, price) => ({
  era: "modern",
  id: equipmentId(name),
  kind: "gear",
  name,
...
const armor = (name, armorValue, price) => ({
  era: "modern",
  id: equipmentId(name),
  kind: "armor",
  name,
...
const weapon = (
  ...
  system: {
    ...
    attackAttributeId:
      skillKey === "melee-combat" ? "reflexes" : "coordination",
    attackBonus: 0,
...
`).map(([name, damage, ammo, short, medium, long, price]) =>
  weapon(
    name,
    damage,
    116,
...
`).map(([name, damage, short, medium, long]) =>
  weapon(
    name,
    damage,
    119,
    ...
    "",
    "strength-damage",
    name.includes("bow") || name.includes("Sling")
      ? "missile-weapons"
      : "throwing",
  ),
);
const explosiveEntries = rows(`
...
const manifestation = (name, skillKey, difficulty, page, tradition) => ({
...
const spells = spellRows.map(([skillName, difficulty, page], index) =>
  manifestation(
...
const psionics = psionicRows.map(([name, difficulty, page], index) =>
  manifestation(
...
const manifestations = [...spells, ...psionics];

const template = (label, page, values, suggestedSkillKeys) => ({
...
const bestiary = (label, page, values, skillScores, move = 10, scale = 0) => ({
  attributeScores: Object.fromEntries(
    attributes.map((id, index) => [id, values[index]]),
  ),
