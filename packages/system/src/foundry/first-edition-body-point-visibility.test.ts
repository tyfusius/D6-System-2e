...
import { createFirstEditionBodyPointRoot } from "../application/first-edition-body-point-root";
...
function message(
  ...
  const root = createFirstEditionBodyPointRoot({
    rootMessageId: "root",
    ...
    operation: "natural",
    before: { current: wound === "stunned" ? 19 : 10, maximum: 20 },
    minutes: 0,
    restModifierScore: 0,
    survivalSource: { attributeId: "brawn" },
  });
  return {
    id: "root",
    author: gm,
    ...
    getFlag: (_namespace: string, key: string) =>
      key === "firstEditionBodyPointRoot"
        ? root
        : key === "bodyPointRootRollMode"
          ? mode
          : undefined,
  } as unknown as FoundryChatMessageDocument;
}
describe("Body Point root Self-roll presentation", () => {
