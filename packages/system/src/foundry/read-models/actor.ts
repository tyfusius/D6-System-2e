...
export function actorRollSources(
  actorValue: object,
): Pick<D6ActorReadModelV1, "attributes" | "skills"> {
...
export function actorReadModel(actorValue: object): D6ActorReadModelV1 {
  return withRuntimeReadScope(() => prepareActorReadModel(actorValue));
}
function prepareActorReadModel(actorValue: object): D6ActorReadModelV1 {
  const actor = actorDocument(actorValue);
  ...
  const optionalCapabilities = currentOptionalCapabilityRuntime();
  const machine = ["starship", "vehicle"].includes(actor.type);
  const { attributes, skills } = actorRollSources(actor);
  const attributeScores = new Map(
    attributes.map((attribute) => [attribute.id, attribute.score]),
  );
