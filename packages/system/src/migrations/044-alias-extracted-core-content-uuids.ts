...
export function aliasExtractedCoreContentUuids(
  source: ActorSource | ItemSource,
): void {
  source.system = rewrite(source.system) as Record<string, unknown>;
  if ("flags" in source) {
    source.flags = rewrite(source.flags);
  }
}

export const aliasExtractedCoreContentUuidsMigration: Migration = Object.freeze(
  {
    name: "Alias extracted Second Edition Core Content UUIDs",
    updateActor: aliasExtractedCoreContentUuids,
    updateItem: aliasExtractedCoreContentUuids,
    version: 44,
  },
);
