...
3. The GM client rejects an expired, malformed, wrong-version, non-owner,
   inactive-player, non-targeted, or expanded-choice request. Canceling the GM
   decision cancels the unresolved roll without posting chat or changing a
   resource.
4. Failed-roll Hero Point rerolls and Doubling Down share one single-use claim
...
5. Claims are serialized per ChatMessage by the designated GM. The first valid
...
6. A GM acting locally uses the same validation and ChatMessage marker without a
   socket round trip. Players cannot use either follow-up without an active GM
   to arbitrate it.
7. Requested rolls use their separate five-minute, versioned protocol. The
...
| GM cancels Wild Die decision             | No chat card, Hero Point transaction, or action completion                                                        | Pending                                                                                                                                                      |
| No active GM                             | Player receives a clear unavailable warning and the unresolved roll stops safely                                  | Pending                                                                                                                                                      |
| Eligible failed roll follow-up           | Owner click is GM-authorized, spends/rolls once, and disables both alternatives on every client                   | Pass — Double Down claimed the source card, created one narrated retry, and the source command rerendered disabled/used on both clients                      |
...
| Follow-up dialog cancellation/failure    | Claim is released and the original commands become usable again                                                   | Pending                                                                                                                                                      |
| Requested-roll delivery                  | Active Tasks shows the remote task; player acknowledgement prevents false failed-delivery state                   | Pass — public Agility request appeared with locked audience while Active Tasks showed Controller online, then cleaned up                                     |
| Player disconnect before acknowledgement | Task changes to failed/offline and GM takeover remains single-completion                                          | Pending                                                                                                                                                      |
| Public / Player + GM / GM-only Blind     | Native Foundry audience and blind redaction match the selected request policy                                     | Partial — Public passed on both clients; private and blind remain pending                                                                                    |
| Reload                                   | Completed chat audit persists; pending Wild/request decisions do not replay                                       | Pending                                                                                                                                                      |
| Console                                  | No system-owned error or deprecation warning                                                                      | Pass — neither client recorded a system-owned warning/error; the player client recorded only Foundry viewport warnings and deliberately failed pre-QA logins |
