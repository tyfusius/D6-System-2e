...
The finite mechanics and pre-Beta content roadmaps are complete. Schema 47 is
...
- Public system version: `0.1.0-alpha.32`; schema: 47.
...
- `npm run release:verify` proves a contiguous migration chain from 001 through
  047, validates the public boundary, and builds a one-entry synthetic private
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| Migration chain              | 47 contiguous files; schema marker, manifest flag, index imports, and loader agree                                                                                                                                                           | Existing development world reaches ready after the planned restart and applies the Adventure Attribute/manifestation boundary                                                                                                                                                                               | Pass   |
| Public packs                 | The base system verifies only the 15-page/43-image manual; all eight official content modules independently verify their owned deterministic packs and lawful public boundaries                                                              | The GM opened all nine Adventure packs alongside the previously accepted extracted modules; Adventure Skills showed 61 records and templates showed ten without moving or rewriting existing package identities                                                                                             | Pass   |
...
| GM startup and reload        | Eleven bundle/loader gates cover the base system, every official content module, both adapters, package/profile registries, and schema 47                                                                                                    | GM retained the Adventure selection, seven-Attribute Actor, and correctly discriminated embedded spell/psionic records through reload; D6 system and adapter initialization completed cleanly                                                                                                               | Pass   |
...
The final `npm run check` result for alpha.32/schema 47 passed 184 test files /
827 tests, all 11 production bundles, all eight official module verifiers, the
15-page/43-image User Manual, clean modular staging, release/invariant checks,
