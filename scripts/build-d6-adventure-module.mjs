...
const traitItem = (entry) =>
  item(`${entry.type}s`, entry.name, entry.type, entry.system);
...
const templateItem = (entry) => {
  const result = item(
    ...
    "character-template",
    {
      ...
      cost: 0,
      description: `Adventure creation template. Apply through Preview & Apply or drag this entry onto a Character sheet during creation. See D6 Adventure, printed p. ${entry.source.page}.`,
      frequency: "always",
      key: entry.id,
      rank: 1,
    },
...
const actor = (entry) => ({
  ...
  items: catalog.packs.skills.map((skill) => {
    const result = skillItem(skill, entry.id);
    const combined = entry.skillScores?.[skill.key];
    if (combined !== undefined)
      result.system.score = Math.max(
        0,
  ...
  system: {
    attributes: Object.fromEntries(
      Object.entries(entry.attributeScores).map(([key, value]) => [
        key,
        { score: value },
      ]),
    ),
...
await Promise.all([
  ...
  pack("advantages", "Item", catalog.packs.advantages.map(traitItem)),
  pack("disadvantages", "Item", catalog.packs.disadvantages.map(traitItem)),
  pack(
    "special-abilities",
    "Item",
    catalog.packs.specialAbilities.map(traitItem),
  ),
